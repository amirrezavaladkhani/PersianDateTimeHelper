...
function jdate($format, $timestamp = '', $none = '', $time_zone = 'Asia/Tehran', $tr_num = 'en')
{
    ...
    for ($i = 0; $i < $sl; $i++) {
        ...
        switch ($sub) {
            ...
            case 'w':
                $out .= ($date[7] == 6) ? 0 : ($date[7] ?? 0) + 1;
                break;

            case 'W':
                $avs = (($date[7] == 6) ? 0 : ($date[7] ?? 0) + 1) - ($doy % 7);
                if ($avs < 0) $avs += 7;
                $num = (int)(($doy + $avs) / 7);
                if ($avs < 4) {
                    $num++;
...
/*	F	*/
function jstrftime($format, $timestamp = '', $none = '', $time_zone = 'Asia/Tehran', $tr_num = 'fa'): array|string
{
...
/*	F	*/
function gregorian_to_jalali($gy, $gm, $gd, $mod = ''): array|string
{
...
/*	F	*/
function jalali_to_gregorian($jy, $jm, $jd, $mod = ''): array|string
{
